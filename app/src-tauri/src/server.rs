...
/// Serve a cached image straight from disk. Used by the preview route,
/// which replaced the asset-protocol approach that proved unreliable on
/// Windows in v1.6.2/v1.6.3.
...
fn validate_preview_token(query: &StreamQuery, token_data: &StreamTokenData) -> bool {
...
async fn thumb_preview_route(
    ...
    if !validate_preview_token(&query, &token_data) {
        return HttpResponse::Forbidden().body("Invalid or missing stream token");
    }
