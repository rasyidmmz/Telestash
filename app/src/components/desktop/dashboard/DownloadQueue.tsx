...
export function DownloadQueue({
    ...
    return (
        <div className="fixed bottom-4 right-4 w-80 bg-stash-surface border border-stash-border rounded-xl shadow-2xl overflow-hidden z-[100]">
            <div className="p-3 border-b border-stash-border bg-stash-hover flex justify-between items-center">
                <div className="flex items-center gap-2">
                    <Download className="w-4 h-4 text-stash-secondary" />
                    <h4 className="text-sm font-medium text-stash-text">Downloads</h4>
                    {activeCount > 0 && (
                        <span className="text-xs px-1.5 py-0.5 bg-stash-secondary/20 text-stash-secondary rounded-full">
                            {activeCount} active
                        </span>
                    )}
                </div>
                <div className="flex gap-2 items-center">
                    ...
                    {completedCount > 0 && (
                        <button onClick={onClearFinished} className="text-xs text-stash-primary hover:text-stash-text transition-colors">
                            Clear Finished
                        </button>
                    )}
                </div>
                    ...
                    <div key={item.id} className="flex flex-col gap-1 p-2 bg-stash-hover rounded">
                        <div className="flex items-center gap-2 text-sm">
                            <div className="flex-shrink-0">
                                ...
                                {item.status === 'downloading' && <div className="w-4 h-4 rounded-full border-2 border-stash-secondary border-t-transparent animate-spin" />}
                            ...
                            <div className="flex-1 truncate text-stash-subtext text-xs" title={item.filename}>
                                {item.filename}
                            </div>
                            ...
                            <>
                                <div className="w-full bg-stash-border h-1 mt-1 rounded-full overflow-hidden">
                                    {item.progress !== undefined ? (
                                        <div
                                            className="bg-stash-secondary h-full rounded-full transition-all duration-300"
                                            style={{ width: `${item.progress}%` }}
                                        />
                                    ) : (
                                        <div className="bg-stash-secondary h-full w-full animate-progress-indeterminate" />
                                    )}
                                </div>
